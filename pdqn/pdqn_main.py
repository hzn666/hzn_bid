...
def rtb(data, budget_para, RL, config, train=True):
    ...
    for day_index, day in enumerate(data['day'].unique()):
        ...
        for slot in range(0, time_fraction):
            ...
            act, act_param, all_action_parameters = RL.select_action(state)
...
def main(budget_para, RL, config):
    ...
    for epoch in range(config['train_epochs']):
        ...
        if test_record[0] > max_clk_on_test:
            max_clk_on_test = test_record[0]
            model_path = glob.glob(os.path.join(record_path, "*.pt"))
            for model in model_path:
                os.remove(model)
            RL.save_models(record_path, epoch)
